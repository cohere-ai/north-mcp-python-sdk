...
def create_x_north_headers(email: str = "test@company.com") -> dict[str, str]:
    ...
    connector_tokens_json = json.dumps({"google": "token123", "slack": "token456"})
    connector_tokens_b64 = base64.urlsafe_b64encode(connector_tokens_json.encode()).decode().rstrip('=')
...
async def test_x_north_headers_success():
    ...
    assert user.connector_access_tokens == {"google": "token123", "slack": "token456"}
...
async def test_x_north_takes_precedence_over_bearer():
    ...
    legacy_token = jwt.encode(payload={"email": "legacy@company.com"}, key="test")
    legacy_header = AuthHeaderTokens(
        server_secret="server_secret",
        user_id_token=legacy_token,
        connector_access_tokens={"legacy": "legacy_token"}
    )
    legacy_b64 = base64.b64encode(json.dumps(legacy_header.model_dump()).encode()).decode()
...
async def test_legacy_bearer_fallback():
    ...
    user_token = jwt.encode(payload={"email": "legacy@company.com"}, key="test")
    legacy_header = AuthHeaderTokens(
        server_secret="server_secret",
